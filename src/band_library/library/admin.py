...
class EntryAdmin(admin.ModelAdmin, ExportCsvMixin):
    ...
    search_fields = ['title', 'composer__given', 'composer__surname', 'arranger__surname', 'callno', 'comments',
                     'composer__realname__surname', 'arranger__realname__surname']
    readonly_fields = ('image_link', 'image_present')
    save_on_top = True
    fields = (('title', 'saleable', 'fee'), ('category', 'callno'), ('genre', 'ensemble', 'key', 'duration'), 'composer',
              'arranger', ('publisher', 'pubyear', 'estdecade', 'platecode'), ('pubname', 'pubissue'), ('source', 'provider'),
              'instrument', ('comments', 'backpage', 'perfnotes'),
              ('material', 'condition', 'completeness', 'duplicate'), 'image_link')
...
class AuthorAdmin(admin.ModelAdmin, ExportCsvMixin):
    ...
    def get_queryset(self, request):
        ...
        qs = qs.annotate(
            _composer=Count('compositions', distinct=True),
            _arranger=Count('arrangements', distinct=True),
            _provider=Count('donations', distinct=True))
